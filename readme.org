...
* How do I build it?
  ...
  With =nix=, it's just =stack install=, without =nix=, install the packages above (or equivalents for your distro) and =stack install --no-nix=.  The build output will indicate the directory (=~/.local/bin=) where the binaries are installed.  You can ignore =elf=, only =ppt= matters.
