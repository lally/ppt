...
import Ppt.Configuration (RunConfig, implSize)
...
-- ch c = do { ws; x <- char c; ws; return x }
...
elementType = ( resvd "double" >> return FDouble )
              <|> ( resvd "int" >> return FInt )
              <|> ( resvd "float" >> return FFloat )
              <|> ( resvd "time" >> return FTime )
              <?> "type name"
